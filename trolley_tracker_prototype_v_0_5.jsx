...
function MapLikeCampus(){
  ...
  const [gpsAccuracy, setGpsAccuracy] = useState(0.85); // 0..1; higher = truer path
      ...
      <CardContent>
        ...
        <div className="flex items-center gap-3 mt-3">
          ...
          <div className="flex gap-1">
            {[1,2,3,4,5].map(n => (
              <button key={n} onClick={()=> setGpsAccuracy(n/5)} className={`h-3 w-5 rounded ${n/5 <= 0.85 ? "bg-green-600" : "bg-neutral-200"}`} />
            ))}
          </div>
